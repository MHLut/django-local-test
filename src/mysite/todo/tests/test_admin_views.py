...
class TodoListAdminViewsTestCase(TestCase):
    ...
    def setUp(self):  # noqa: D102
        ...
        self.item_two = TodoItem.objects.create(todo_list=self.list, description="Two", completed_at=timezone.now())
        self.item_three = TodoItem.objects.create(todo_list=self.list, description="Three")

        self.default_view_paths = [
            # TodoList:
            ("admin:todo_todolist_changelist", None),
            ("admin:todo_todolist_add", None),
            ("admin:todo_todolist_change", {"object_id": self.list.pk}),
            ("admin:todo_todolist_delete", {"object_id": self.list.pk}),
            ("admin:todo_todolist_history", {"object_id": self.list.pk}),
            # TodoItem:
            ("admin:todo_todoitem_changelist", None),
            ("admin:todo_todoitem_add", None),
            ("admin:todo_todoitem_change", {"object_id": self.item_one.pk}),
